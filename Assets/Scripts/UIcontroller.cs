...
public class UIcontroller : MonoBehaviour
{
    ...
    private void
        Awake() //Awake вызывается в самом начале игры один раз, и вместе с ним активируется менюПанель, и не будут активны gamePanel и WinPanel
    {
    ...
    private void
        OnPLay() //метод который включает GamePanel, Action - ивент(рация) на которую мы подписались, которая известит о нажатии кнопки "старт гейм" => и как следствие при нажатии кнопки "старт гейм" отключает МенюПанель и включет ГеймПанель
    {
    ...
    private void
        NewGame() //аналогично OnPlay // включается при нажатии кнопки "о переходе на след уровень" на панелиПобеды , т е включает панель игры и отключает остальные панели 
    {
        _menuPanel.gameObject.SetActive(false);
        _gamePanel.gameObject.SetActive(true);
        _winPanel.gameObject.SetActive(false);
